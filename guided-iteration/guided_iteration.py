...
def get_data(selected_efps, include_hl, include_pt):
    # Data path
    ...
    pT = pd.DataFrame(np.load(data_path / "raw" / "muon_pTs.npy"))
...
def train_nn(ix):
    ...
    X, y = get_data(
        selected_efps=selected_efps, include_hl=include_hl, include_pt=include_pt
    )
